...
const IconBackground = () => {
  ...
  return (
    <div
      ref={containerRef}
      className="fixed inset-0 z-0 overflow-hidden bg-[var(--bg-main)] opacity-40 pointer-events-none"
    >
      <div
...
const IconCell = ({ Icon, mousePos, index, totalCols }: { Icon: any, mousePos: { x: number, y: number }, index: number, totalCols: number }) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      if (cellRef.current) {
        ...
        // Wave effect logic
        // Max scale 2.5 when cursor is close (within 150px)
        // Decay to scale 1 as distance increases
        const maxDist = 250;
        const maxScale = 2.2;
  ...
  return (
    <div
      ref={cellRef}
      className="flex items-center justify-center text-[var(--text-secondary)]/20"
    >
      <Icon
        style={{
          transform: `scale(${scale})`,
          opacity: 0.2 + (scale - 1) * 0.3, // Increase opacity as it scales up
